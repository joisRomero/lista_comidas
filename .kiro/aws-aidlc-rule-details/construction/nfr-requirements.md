...
## Overview
Determine non-functional requirements for the unit and make tech stack choices. If organizational standards were defined during Requirements Analysis, those are HARD CONSTRAINTS — do not re-ask or suggest alternatives for constrained areas.

## Steps to Execute

### Step 1: Load Constraints and Context
#### Load Organizational Standards from Requirements
- [ ] Load `aidlc-docs/inception/requirements/requirements.md`
- [ ] Check for **Organizational Standards** section — if present, these are HARD CONSTRAINTS:
  - Do NOT ask questions about constrained areas (e.g., if DB is defined as SQL Server, do not ask "which database?")
  - Do NOT suggest alternatives to constrained technologies
  - DO use constraints to pre-fill tech stack decisions
  - DO ask questions ONLY about areas left open (not constrained)
- [ ] If no Organizational Standards section exists, all technology choices are open for evaluation

#### Load Security Context from Inception
- [ ] Load ISO 27001 assessment from `aidlc-docs/inception/iso-27001/iso-27001-assessment.md` (if available)
- [ ] Use applicable control domains to inform security requirement questions
- [ ] Incorporate security considerations identified during Inception assessment

#### Load Functional Design
...
- **Tech Stack Selection** - Ask about technology preferences, constraints, existing systems, and integration requirements. **SKIP questions for any technology already defined as a HARD CONSTRAINT in Organizational Standards** — only ask about areas left open.
