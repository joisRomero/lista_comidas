...
- [ ] **Multi-Repo only**: If `aidlc-docs/inception/plans/hu-repo-distribution.md` exists, include a **Repos Impacted** column in the story map linking each HU to its target repositories. This ensures unit boundaries align with repo boundaries.
...
- [ ] **Multi-Repo only**: Validate that unit boundaries do not create cross-repo dependencies within a single unit (each unit should map cleanly to repos)
